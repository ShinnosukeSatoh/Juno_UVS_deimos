...
F.set_yaxis(ax_idx=0,
            label='$M$ [10$^{-9}$ kg m$^{-2}$]',
            min=0, max=ymax,
            ticks=np.arange(10, 40+1, 10),
            ticklabels=np.arange(10, 40+1, 10),
            minor_num=5)
...
F.set_yaxis(ax_idx=2, label='Brightnees [R]',
            min=0, max=300,
            ticks=np.arange(0, 300+1, 100)[1:],
            ticklabels=np.arange(0, 300+1, 100)[1:],
            minor_num=5)
F.set_yaxis(ax_idx=3, label='Brightnees [R]',
            min=0, max=300,
            ticks=np.arange(0, 300+1, 100),
            ticklabels=np.arange(0, 300+1, 100),
            minor_num=5)
...
# %% Hisaki & Morgenthaler+ 2024の結果と比較
F = ShareXaxis()
F.fontsize = 22
F.fontname = 'Liberation Sans Narrow'

F.set_figparams(nrows=5, figsize=(8.5, 10.5), dpi='L')
F.initialize()
# F.panelname = [' a. Io ', ' b. Europa ', ' c. Ganymede ']
...
# Hisaki data
F.set_yaxis(ax_idx=2, label='Intensity\n[GW]',
            min=0, max=400,
            ticks=np.linspace(0, 400, 5)[1:],
            ticklabels=np.linspace(0, 400, 5, dtype=int)[1:],
            minor_num=5)
...
F.ax[2].scatter(datetime_dusk, L3torus_dusk[:, 1],
                s=1.0, marker='.', color=UC.red,
                label='Dusk')
F.ax[2].scatter(datetime_dawn, L3torus_dawn[:, 1],
                s=1.0, marker='.', color=UC.blue,
                label='Dawn')

# 4th & 5th axis
F.set_yaxis(ax_idx=3, label='Brightnees\n[R]',
            min=0, max=300,
            ticks=np.arange(0, 300+1, 100)[1:],
            ticklabels=np.arange(0, 300+1, 100)[1:],
            minor_num=5)
F.set_yaxis(ax_idx=4, label='Brightnees\n[R]',
            min=0, max=300,
            ticks=np.arange(0, 300+1, 100),
            ticklabels=np.arange(0, 300+1, 100),
            minor_num=5)
...
F.ax[3].scatter(tavg_datetime, ansa_right_brightness,
                s=1, marker='.', fc=UC.red, label='Dusk')
F.ax[3].scatter(tavg_datetime, ansa_left_brightness, s=1,
                marker='.', fc=UC.blue, label='Dawn')
F.ax[3].errorbar(x=tavg_datetime, y=ansa_right_brightness,
                 yerr=ansa_right_berr,
                 elinewidth=0.3, linewidth=0., markersize=0,
                 color=UC.red)
F.ax[3].errorbar(x=tavg_datetime, y=ansa_left_brightness,
                 yerr=ansa_left_berr,
                 elinewidth=0.3, linewidth=0., markersize=0,
                 color=UC.blue)
...
for i in range(data2_len):
    ...
    tavg_datetime2 += [t.tt.datetime]
cmap_turbo = plt.get_cmap('turbo')
F.ax[4].scatter(tavg_datetime2, r_60rj_biweight,
                s=1, marker='.', fc=cmap_turbo(30),
                label=r'6.0 $R_{\rm J}$')
F.ax[4].scatter(tavg_datetime2, r_120rj_biweight,
                s=1, marker='.', fc=cmap_turbo(100),
                label=r'12.0 $R_{\rm J}$')
F.ax[4].scatter(tavg_datetime2, r_240rj_biweight,
                s=1, marker='.', fc=cmap_turbo(170),
                label=r'24.0 $R_{\rm J}$')
...
F.textbox(ax_idx=1, x=0.50, y=0.89,
          ...
          edgecolor=(0, 0, 0, 0), )
F.textbox(ax_idx=2, x=0.13, y=0.88,
          text='Hisaki', fontsize=F.fontsize*0.63,
          horizontalalignment='center',
          facealpha=0.5, facecolor='k',
          transform=F.ax[2].transAxes,
          edgecolor=(0, 0, 0, 1.0), )
F.textbox(ax_idx=3, x=0.13, y=0.88,
          ...
          transform=F.ax[3].transAxes,
          edgecolor=(0, 0, 0, 1.0), )
F.textbox(ax_idx=4, x=0.13, y=0.88,
          text='Na nebula', fontsize=F.fontsize*0.63,
          horizontalalignment='center',
          facealpha=0.5, facecolor='k',
          transform=F.ax[4].transAxes,
          edgecolor=(0, 0, 0, 1.0), )
...
legend = F.legend(ax_idx=4,
                  ncol=3, markerscale=10.0,
                  loc='upper right',
                  handlelength=1.0,
                  fontsize_scale=0.65, handletextpad=0.2)
legend_shadow(legend=legend, fig=F.fig, ax=F.ax[4], d=0.7)

F.fig.savefig('img/ftmc_Io_hisaki_jpm2024.pdf', bbox_inches='tight')
plt.show()
