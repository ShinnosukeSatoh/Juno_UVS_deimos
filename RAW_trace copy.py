...
def calc_copy(Ai, ni, Hp, r_t0, s3wlon_t0, z_t0, s_t0, hem, reflect_altitude):
    ...
    tau_N_skip = tau_N[0]
    s3wlon_N_skip = s3wlon_N[0]
    theta_s3_N_skip = theta_s3_N[0]
    alt_flag_N_skip = alt_flag_N[0]
    for i in range(int(non_0.size/2)):
        ...
        alt_flag_N_skip = np.hstack((alt_flag_N_skip,
                                     alt_flag_N[non_0[i]]))
    tau_N_skip = np.hstack((tau_N_skip,
                            tau_N[non_0[i]+1:non_0[i+1]][::skip]))
    s3wlon_N_skip = np.hstack((s3wlon_N_skip,
                               s3wlon_N[non_0[i]+1:non_0[i+1]][::skip]))
    theta_s3_N_skip = np.hstack((theta_s3_N_skip,
                                 theta_s3_N[non_0[i]+1:non_0[i+1]][::skip]))
    alt_flag_N_skip = np.hstack((alt_flag_N_skip,
                                 alt_flag_N[non_0[i]+1:non_0[i+1]][::skip]))
    for i in range(int(non_0.size/2)):
        i += int(non_0.size/2)
        tau_N_skip = np.hstack((tau_N_skip,
                                tau_N[non_0[i]]))
        ...
        alt_flag_N_skip = np.hstack((alt_flag_N_skip,
                                     alt_flag_N[non_0[i]]))
    tau_N_skip = np.hstack((tau_N_skip, tau_N[-1]))
    s3wlon_N_skip = np.hstack((s3wlon_N_skip, s3wlon_N[-1]))
    theta_s3_N_skip = np.hstack((theta_s3_N_skip, theta_s3_N[-1]))
    alt_flag_N_skip = np.hstack((alt_flag_N_skip, alt_flag_N[-1]))
    ...
    tau_S_skip = tau_S[0]
    s3wlon_S_skip = s3wlon_S[0]
    theta_s3_S_skip = theta_s3_S[0]
    alt_flag_S_skip = alt_flag_S[0]
    for i in range(int(non_0.size/2)):
        ...
        alt_flag_S_skip = np.hstack((alt_flag_S_skip,
                                     alt_flag_S[non_0[i]]))
    tau_S_skip = np.hstack((tau_S_skip,
                            tau_S[non_0[i]+1:non_0[i+1]][::skip]))
    s3wlon_S_skip = np.hstack((s3wlon_S_skip,
                               s3wlon_S[non_0[i]+1:non_0[i+1]][::skip]))
    theta_s3_S_skip = np.hstack((theta_s3_S_skip,
                                 theta_s3_S[non_0[i]+1:non_0[i+1]][::skip]))
    alt_flag_S_skip = np.hstack((alt_flag_S_skip,
                                 alt_flag_S[non_0[i]+1:non_0[i+1]][::skip]))
    for i in range(int(non_0.size/2)):
        i += int(non_0.size/2)
        tau_S_skip = np.hstack((tau_S_skip,
                                tau_S[non_0[i]]))
        ...
        alt_flag_S_skip = np.hstack((alt_flag_S_skip,
                                     alt_flag_S[non_0[i]]))
    tau_S_skip = np.hstack((tau_S_skip, tau_S[-1]))
    s3wlon_S_skip = np.hstack((s3wlon_S_skip, s3wlon_S[-1]))
    theta_s3_S_skip = np.hstack((theta_s3_S_skip, theta_s3_S[-1]))
    alt_flag_S_skip = np.hstack((alt_flag_S_skip, alt_flag_S[-1]))
    ...
    tau_N2S_skip = tau_N2S[0]
    s3wlon_N2S_skip = s3wlon_N2S[0]
    theta_s3_N2S_skip = theta_s3_N2S[0]
    alt_flag_N2S_skip = alt_flag_N2S[0]
    for i in range(int(non_0.size/2)):
    ...
    for i in range(int(non_0.size/2)):
        ...
        alt_flag_N2S_skip = np.hstack((alt_flag_N2S_skip,
                                       alt_flag_N2S[non_0[i]]))
    tau_N2S_skip = np.hstack((tau_N2S_skip, tau_N2S[-1]))
    s3wlon_N2S_skip = np.hstack((s3wlon_N2S_skip, s3wlon_N2S[-1]))
    theta_s3_N2S_skip = np.hstack((theta_s3_N2S_skip, theta_s3_N2S[-1]))
    alt_flag_N2S_skip = np.hstack((alt_flag_N2S_skip, alt_flag_N2S[-1]))
...
def main():
    ...
    # tau
    # =======================================
    tau_N_MAW = results_list[s3wlon_180][0][0][:]
    tau_S_MAW = results_list[s3wlon_180][1][0][:]
    tau_N2S = results_list[s3wlon_180][2][0][:]
    tau_S2N = results_list[s3wlon_180][3][0][:]

    # =======================================
    # colatitude as a function of tau
    # =======================================
    theta_N_MAW = results_list[s3wlon_180][0][2][:]
    theta_S_MAW = results_list[s3wlon_180][1][2][:]
    theta_N2S = results_list[s3wlon_180][2][2][:]
    theta_S2N = results_list[s3wlon_180][3][2][:]
    ...
    # ==========================================
    tau_hR_N_hR = tau_S_MAW[-reflect_alt_target]*2
    tau_hR_S_hR = tau_N_MAW[-reflect_alt_target]*2
    ...
    print('theta_N_1.shape:', theta_N_1.shape)
    print('eqlead_N_1.shape:', eqlead_N_1.shape)
    ...
    ax.plot(eqlead_N_0, np.cos(theta_N_0),
            color=UC.red)
    ax.plot(eqlead_N_1, np.cos(theta_N_1),
            color=UC.red)
    ax.plot(eqlead_N_2, np.cos(theta_N_2),
            color=UC.red)
    ax.plot(eqlead_N_3, np.cos(theta_N_3),
            color=UC.red)
    ax.plot(eqlead_N_4, np.cos(theta_N_4),
            color=UC.red)
    ax.plot(eqlead_S_0, np.cos(theta_S_0),
            color=UC.blue)
    ax.plot(eqlead_S_1, np.cos(theta_S_1),
            color=UC.blue)
    ax.plot(eqlead_S_2, np.cos(theta_S_2),
            color=UC.blue)
    ax.plot(eqlead_S_3, np.cos(theta_S_3),
            color=UC.blue)
    ax.plot(eqlead_S_4, np.cos(theta_S_4),
            color=UC.blue)
    fig.tight_layout()
    fig.savefig('img/reflect/eqlead_vs_s3lat_.jpg')
    plt.close()
...
if __name__ == '__main__':
    ...
    alt_ref = [1500.0, 900.0, 400.0, 5.0]
    reflect_alt_target = -4       # ALWAYS NEGATIVE!!!
